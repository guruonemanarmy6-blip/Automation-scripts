...
def process_reports():
    with sync_playwright() as p:
        ...
        # -----------------------------------------------
        # 1. LEVEL 18 PURE V8 OMNI-SORTING
        # -----------------------------------------------
        ...
        # -----------------------------------------------
        # 2. LEVEL 18 PURE V8 FILTERING (NO IPC LOOPING)
        # -----------------------------------------------
        ...
        try:
            for idx, item in enumerate(REPORTS_LIST):
                try:
                    ...
                    # -----------------------------------------------
                    # 3. LEVEL 18 PURE V8 CROPPING ENGINE (NO IPC LOOPING)
                    # -----------------------------------------------
                    ...
                    if crop_box and crop_box.get('height', 0) > 100 and crop_box.get('width', 0) > 100:
                        ...
                        page.screenshot(path=file_path, clip={'x': x, 'y': y, 'width': width, 'height': height}, timeout=25000)
